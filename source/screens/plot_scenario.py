...
class PlotPageScenarioTransaction:
    """One reversible preparation of Plot's ordinary visible activation.

    Unlike :class:`PlotScenarioTransaction`, this lease never starts a curve
    job, discards a program, reserves workspace, or draws a pixel.  It only
    prepares the existing editor and deferred curve intent exactly as normal
    navigation activation would.
    """

    __slots__ = (
        "_screen", "_closed", "_stepped", "_mode", "_overlay_y",
        "_input_str", "_input_cursor_pos", "_input_view_offset",
        "_cursor_x", "_cursor_y", "_cursor_width", "_cursor_height",
        "_cursor_mode", "_cursor_visible", "_cursor_gs",
        "_needs_curve_restore", "_curve_restore_auto_scale")
    ...
    def __init__(self, screen):
        ...
        # This activation lease retains only editor text.  Validate it before
        # installing the shared guard so oversized direct assignments cannot
        # turn a failed open into a partially claimed Plot page.
        _require_plot_snapshot_text(input_str)
        ...
        # Every saved value is a scalar or a pre-existing direct reference.
        # Keep curve, program, job, and workspace ownership on the screen.
        screen._state[1][2] = self

    def _require_open(self):
        screen = self._screen
        if self._closed or screen is None:
            raise RuntimeError("Plot page scenario transaction is closed")
        if screen._state[1][2] is not self:
            raise RuntimeError("Plot page scenario transaction is not active")
        return screen

    def step(self):
        """Prepare the ordinary Plot activation state exactly once."""
        screen = self._require_open()
        if self._stepped:
            raise RuntimeError("Plot page scenario transaction is prepared")
        screen._activate_visible_state()
        self._stepped = True
        return True

    def _close(self):
        """Restore direct semantic state and release lease references."""
        if self._closed:
            return True
        screen = self._require_open()
        box = screen.input_box
        ...
        screen._state[1][3] = self._mode
        screen._state[3][3][0][3] = self._overlay_y
        screen._state[2][2] = self._needs_curve_restore
        screen._state[2][3] = self._curve_restore_auto_scale
        screen._clear_presented_editor_state()
        screen._state[1][2] = None
        self._screen = None
        self._input_str = None
    ...
    def close_with_primary(self, primary_error):
        """Restore while preserving an action error over ordinary cleanup.

        A primary ``MemoryError`` wins over a restore OOM by object identity;
        an ordinary primary is upgraded to a restore OOM.  A failed restore
        leaves the active guard and snapshots untouched for a later retry.
        """
        try:
            self._close()
        except MemoryError:
    ...
    def close(self):
        """Restore direct semantic state; zero-argument callers stay compatible."""
        return self.close_with_primary(None)
