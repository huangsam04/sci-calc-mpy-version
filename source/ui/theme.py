...
def draw_text(display, x, y, text, font=None, gs=GS_TEXT, invert=False, raw=False):
    if font:
        display.draw_text(x, y, text, font, invert=invert, gs=gs, raw=raw)
    else:
        display.draw_text8x8(x, y, text, gs=0 if invert else gs)


def draw_header(display, title, font=None):
    draw_text(display, TEXT_X, TITLE_Y, fit_text(title, CONTENT_W - 6, font), font)
    display.draw_hline(0, TITLE_LINE_Y, CONTENT_W, GS_LINE)


def draw_footer(display, hint, font=None, right=""):
    display.fill_rectangle(0, FOOTER_Y, CONTENT_W, SCREEN_H - FOOTER_Y, 0)
    display.draw_hline(0, FOOTER_Y, CONTENT_W, GS_LINE)
    draw_text(display, TEXT_X, FOOTER_Y + 2, fit_text(hint, 126, font), font, GS_MUTED)
    if right:
