...
取消编辑会立即恢复提交前表达式及其光标位置；离开 Plot 或打开子页面也会按当前文本重建光标几何，
再次进入编辑时不会短暂显示隐藏状态恢复过程中的中间位置。正常输入与 Calculator 复用同一个
96 ms ease-out 调度路径，光标会从首次 2 px 反馈继续移动到完整 8 px 字符终点。
