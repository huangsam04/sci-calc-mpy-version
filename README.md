...
表达式只编译一次，每个像素位置也只采样一次。自动缩放通常保留完整极值；当少量
...
固定条带仅 512 字节，状态栏不参与页面揭示。转场和控件运动使用统一的非线性
ease-out 曲线，结束时主帧缓冲已经是完整实时页面。静止页面只在
...
2026-07-23 在 COM5 上对 512 字节条带版本连续做了 200 次页面往返。GC p95 为22.9 ms，
结束堆仅下降256字节；动画帧平均5.8 ms、最慢12.3 ms，最低空闲堆4,640字节且无
`MemoryError`。当前版本进一步取消 Plot 和 FunctionPanel 的直切：旧页状态先释放，动画只揭示
目标页的默认空布局，曲线、插件菜单和页面快照在动画结束后恢复；条带分配失败时使用 OLED
电流淡入淡出。可用 `mpremote ... run tools\device_runtime_monitor.py` 重新采集实机结果。
