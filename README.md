...
[性能调优留痕](docs/PERFORMANCE_TUNING_2026-07-23.md)。
...
2026-07-23 在 COM5 上预热后连续 50 次页面往返的事件到首帧 p95 为 115.8 ms，转场帧
p95 为 28.5 ms、全程最大值为 86.3 ms，GC p95 为 23.7 ms，结束时堆比采样前多出
10,256 字节可用空间。页面滑动会改变几乎整个内容区；SSD1322 的四像素列对齐会进一步
扩大矩形，且增量传输需要额外比较和逐行 SPI 事务。因此当前保留全帧传输，而不引入会降低
完整页面转场性能且额外占用堆的脏矩形缓存。
