...
- [ ] 在 COM5 比较冻结前后相同最大状态与五轮场景。一次只调整一个冻结集合；无稳定收益、导致 Flash 超限或破坏动态 Add-ons 的集合立即移除。
...
- [ ] 冻结固件可冷启动并运行现有 Calculator、Plot、Function Panel、Stopwatch、Settings、恢复路径和用户 Add-ons。
- [ ] 一个 8192 B framebuffer、0 B 新像素缓冲、逐帧 0 分配和热路径无 GC 查询/收集保持成立。
- [ ] 记录冻结前后实际最低空闲堆、启动时间、最大 step、Flash 占用和固件构建/刷写耗时；只有真机证明的净收益进入下一分支。
- [ ] COM5 数据和用户文件保持不变，验收结束后 OLED 已休眠。
